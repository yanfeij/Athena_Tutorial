...
void Vacuum(MeshBlock *pmb, Coordinates *pco, NRRadiation *prad,
          const AthenaArray<Real> &w, FaceField &b,
          AthenaArray<Real> &ir,
          Real time, Real dt, int is, int ie, int js, int je, int ks, int ke, int ngh);
...
void MeshBlock::InitUserMeshBlockData(ParameterInput *pin)
{
  ...
  reset_flag = pin->GetOrAddInteger("problem","reset",0);

  if(NR_RADIATION_ENABLED || IM_RADIATION_ENABLED)
    pnrrad->EnrollOpacityFunction(FFOpacity);
    ...
    for(int k=0; k<ncells3; ++k) {
      for (int j=0; j<ncells2; ++j) {
        for (int i=0; i<ncells1; ++i) {
          ...
          prad->sigma_pe(k,j,i,0) = prad->sigma_p(k,j,i,0);
    ...
    for (int j=js; j<=je; ++j) {
      for (int i=1; i<=ngh; ++i) {
        ...
        for(int ifr=0; ifr<nfreq; ++ifr){
          if((nfreq == 1) || (nfreq > 1 && ifr == nfreq-1)){
...
void FFOpacity(MeshBlock *pmb, AthenaArray<Real> &prim)
{
  ...
  for (int i=il; i<=iu; ++i) {
    ...
    if(prad->nfreq > 1){
      ...
      prad->sigma_p(k,j,i,1) = prad->sigma_a(k,j,i,1);
      prad->sigma_pe(k,j,i,1) = prad->sigma_p(k,j,i,1);
    }
